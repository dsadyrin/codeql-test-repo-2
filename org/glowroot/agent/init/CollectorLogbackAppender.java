...
class CollectorLogbackAppender extends AppenderBase<ILoggingEvent> {
    ...
    protected void append(ILoggingEvent event) {
        ...
        if (event.getLevel() == Level.DEBUG && event.getLoggerName().startsWith("org.glowroot.agent.shaded.grpc.")) {
            return;
        }
        if (event.getLoggerName().startsWith("org.glowroot.central.")
                || event.getLoggerName().startsWith("org.glowroot.agent.shaded.glowroot.ui.")) {
