package org.glowroot.agent.shaded.qos.logback.classic.layout;

import org.glowroot.agent.shaded.qos.logback.classic.pattern.ThrowableProxyConverter;
...
import org.glowroot.agent.shaded.qos.logback.core.util.CachingDateFormatter;

/**
 * A layout with a fixed format. The output is equivalent to that produced by {@link org.glowroot.agent.shaded.qos.logback.classic.PatternLayout PatternLayout} with the pattern:</p>
 * 
 * <pre>%d{HH:mm:ss.SSS} [%thread] %-5level %logger{36} - %msg%n</pre>
 * 
 *<p>TTLLLayout has the advantage of faster load time whereas {@link org.glowroot.agent.shaded.qos.logback.classic.PatternLayout PatternLayout}
