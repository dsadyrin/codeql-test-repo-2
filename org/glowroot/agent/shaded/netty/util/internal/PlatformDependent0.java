...
final class PlatformDependent0 {
    ...
    private static boolean explicitNoUnsafe0() {
        final boolean noUnsafe = SystemPropertyUtil.getBoolean("org.glowroot.agent.shaded.netty.noUnsafe", false);
        logger.debug("-Dio.netty.noUnsafe: {}", noUnsafe);

        if (noUnsafe) {
            logger.debug("sun.misc.Unsafe: unavailable (org.glowroot.agent.shaded.netty.noUnsafe)");
            return true;
        }

        // Legacy properties
        boolean tryUnsafe;
        if (SystemPropertyUtil.contains("org.glowroot.agent.shaded.netty.tryUnsafe")) {
            tryUnsafe = SystemPropertyUtil.getBoolean("org.glowroot.agent.shaded.netty.tryUnsafe", true);
        } else {
            tryUnsafe = SystemPropertyUtil.getBoolean("org.jboss.netty.tryUnsafe", true);
        }

        if (!tryUnsafe) {
            logger.debug("sun.misc.Unsafe: unavailable (org.glowroot.agent.shaded.netty.tryUnsafe/org.jboss.netty.tryUnsafe)");
            return true;
        }
